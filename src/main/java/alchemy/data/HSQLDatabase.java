...
public class HSQLDatabase implements IStubDatabase {
    ...
    public void createTables() throws SQLException {
        try (Connection conn = getConnection();
            ...
            // Create holdfasts table.
            String createHoldfastsTable = "CREATE TABLE IF NOT EXISTS PUBLIC.HOLDFASTS ("
                    + "id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
                    + "group_name VARCHAR(255) NOT NULL UNIQUE, "
                    + "holdfast_name VARCHAR(255) NOT NULL, "
                    + "base_gold_per_day DOUBLE DEFAULT 40.0, "
                    + "population INTEGER DEFAULT 40, "
                    + "castle_type VARCHAR(50) DEFAULT 'wood_fort', "
                    + "gold DOUBLE DEFAULT 0.0, "
                    + "silver INTEGER DEFAULT 0, "
                    + "happiness DOUBLE DEFAULT 50.0, "
                    + "target_happiness DOUBLE DEFAULT 50.0, "
                    + "days_elapsed INTEGER DEFAULT 0, "
                    + "beer INTEGER DEFAULT 0, "
                    + "grain INTEGER DEFAULT 0, "
                    + "wine INTEGER DEFAULT 0, "
                    + "tools INTEGER DEFAULT 0, "
                    + "raids_survived INTEGER DEFAULT 0, "
                    + "population_growth_history VARCHAR(2000) DEFAULT ''"
                    + ")";
            stmt.executeUpdate(createHoldfastsTable);

            String createHoldfastBuildingsTable = "CREATE TABLE IF NOT EXISTS PUBLIC.HOLDFAST_BUILDINGS ("
                    + "holdfast_id INTEGER NOT NULL, "
                    + "building_type VARCHAR(100) NOT NULL, "
                    + "count INTEGER DEFAULT 0, "
                    + "PRIMARY KEY (holdfast_id, building_type), "
                    + "FOREIGN KEY (holdfast_id) REFERENCES PUBLIC.HOLDFASTS(id)"
                    + ")";
            stmt.executeUpdate(createHoldfastBuildingsTable);

            String createHoldfastPlantDaysTable = "CREATE TABLE IF NOT EXISTS PUBLIC.HOLDFAST_PLANT_DAYS ("
                    + "holdfast_id INTEGER NOT NULL, "
                    + "field_type VARCHAR(50) NOT NULL, "
                    + "plant_day INTEGER NOT NULL, "
                    + "sort_order INTEGER DEFAULT 0 NOT NULL, "
                    + "FOREIGN KEY (holdfast_id) REFERENCES PUBLIC.HOLDFASTS(id)"
                    + ")";
            stmt.executeUpdate(createHoldfastPlantDaysTable);

            String createHoldfastEventsTable = "CREATE TABLE IF NOT EXISTS PUBLIC.HOLDFAST_EVENTS ("
                    + "id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
                    + "holdfast_id INTEGER NOT NULL, "
                    ...
                    + "FOREIGN KEY (holdfast_id) REFERENCES PUBLIC.HOLDFASTS(id)"
                    + ")";
            stmt.executeUpdate(createHoldfastEventsTable);

            // Migration: add new resource columns if they don't exist yet
            String[] newCols = {
...
public void deletePlayer(int playerId) throws SQLException {
    ...
    public void addHoldfast(Holdfast h) {
        ...
        try (Connection conn = getConnection();
            ...
            ps.setString(2, h.getHoldfastName());
            ps.setDouble(3, h.getBaseGoldPerDay());
            ps.setInt(4, h.getPopulation());
            ps.setString(5, h.getCastleType());
            ps.setDouble(6, h.getGold());
            ps.setInt(7, h.getSilver());
            ps.setDouble(8, h.getHappiness());
            ps.setDouble(9, h.getTargetHappiness());
            ps.setInt(10, h.getDaysElapsed());
            ps.setInt(11, h.getBeer());
            ps.setInt(12, h.getGrain());
            ps.setInt(13, h.getWine());
            ps.setInt(14, h.getTools());
            ps.setInt(15, h.getRaidsSurvived());
            ps.setString(16, joinInts(h.getPopulationGrowthHistory()));
            ps.setInt(17, h.getFood());
            ...
            insertBuildingsAndPlantDays(conn, h);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }

    private void insertBuildingsAndPlantDays(Connection conn, Holdfast h) throws SQLException {
        // Insert non-zero buildings
        String bSql = "INSERT INTO PUBLIC.HOLDFAST_BUILDINGS (holdfast_id, building_type, count) VALUES (?, ?, ?)";
        ...
        // Insert plant days
        String pdSql = "INSERT INTO PUBLIC.HOLDFAST_PLANT_DAYS (holdfast_id, field_type, plant_day, sort_order) VALUES (?, ?, ?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(pdSql)) {
            insertPlantDaysBatch(ps, h.getId(), "wheat_field", h.getWheatFieldPlantDays());
    ...
    private void insertPlantDaysBatch(PreparedStatement ps, int holdfastId, String fieldType, List<Integer> days) throws SQLException {
        for (int i = 0; i < days.size(); i++) {
            ps.setInt(1, holdfastId);
            ps.setString(2, fieldType);
            ps.setInt(3, days.get(i));
            ps.setInt(4, i);
            ps.addBatch();
        }
    }
    ...
    public Holdfast getHoldfast(String groupName) {
        String sql = "SELECT * FROM PUBLIC.HOLDFASTS WHERE group_name = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, groupName);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return buildHoldfastFromResultSet(conn, rs);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    private Holdfast buildHoldfastFromResultSet(Connection conn, ResultSet rs) throws SQLException {
        Holdfast h = new Holdfast();
        h.setId(rs.getInt("id"));
        h.setGroupName(rs.getString("group_name"));
        h.setHoldfastName(rs.getString("holdfast_name"));
        h.setBaseGoldPerDay(rs.getDouble("base_gold_per_day"));
        h.setPopulation(rs.getInt("population"));
        h.setCastleType(rs.getString("castle_type"));
        h.setGold(rs.getDouble("gold"));
        h.setSilver(rs.getInt("silver"));
        h.setHappiness(rs.getDouble("happiness"));
        h.setTargetHappiness(rs.getDouble("target_happiness"));
        h.setDaysElapsed(rs.getInt("days_elapsed"));
        h.setBeer(rs.getInt("beer"));
        h.setGrain(rs.getInt("grain"));
        h.setWine(rs.getInt("wine"));
        h.setTools(rs.getInt("tools"));
        h.setRaidsSurvived(rs.getInt("raids_survived"));
        String growthHistory = rs.getString("population_growth_history");
        h.setPopulationGrowthHistory(splitInts(growthHistory));
        h.setFood(rs.getInt("food"));
        ...
        try (PreparedStatement bps = conn.prepareStatement(bSql)) {
            bps.setInt(1, h.getId());
            try (ResultSet brs = bps.executeQuery()) {
                while (brs.next()) {
                    h.setBuildingCount(brs.getString("building_type"), brs.getInt("count"));
                }
            }
        }

        // Load plant days
        String pdSql = "SELECT field_type, plant_day FROM PUBLIC.HOLDFAST_PLANT_DAYS WHERE holdfast_id = ? ORDER BY field_type, sort_order";
        try (PreparedStatement pdps = conn.prepareStatement(pdSql)) {
            pdps.setInt(1, h.getId());
            try (ResultSet pdrs = pdps.executeQuery()) {
                while (pdrs.next()) {
                    String fieldType = pdrs.getString("field_type");
                    int plantDay = pdrs.getInt("plant_day");
                    switch (fieldType) {
                        case "wheat_field": h.getWheatFieldPlantDays().add(plantDay); break;
    ...
    public List<Holdfast> getAllHoldfasts() {
        List<Holdfast> result = new ArrayList<>();
        String sql = "SELECT group_name FROM PUBLIC.HOLDFASTS";
        try (Connection conn = getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
            while (rs.next()) {
                Holdfast h = getHoldfast(rs.getString("group_name"));
    ...
    public void updateHoldfast(Holdfast h) {
        ...
        try (Connection conn = getConnection();
            ...
            ps.setDouble(2, h.getBaseGoldPerDay());
            ps.setInt(3, h.getPopulation());
            ps.setString(4, h.getCastleType());
            ps.setDouble(5, h.getGold());
            ps.setInt(6, h.getSilver());
            ps.setDouble(7, h.getHappiness());
            ps.setDouble(8, h.getTargetHappiness());
            ps.setInt(9, h.getDaysElapsed());
            ps.setInt(10, h.getBeer());
            ps.setInt(11, h.getGrain());
            ps.setInt(12, h.getWine());
            ps.setInt(13, h.getTools());
            ps.setInt(14, h.getRaidsSurvived());
            ps.setString(15, joinInts(h.getPopulationGrowthHistory()));
            ps.setInt(16, h.getFood());
            ...
            try (PreparedStatement del = conn.prepareStatement("DELETE FROM PUBLIC.HOLDFAST_BUILDINGS WHERE holdfast_id = ?")) {
                del.setInt(1, h.getId());
                del.executeUpdate();
            }
            try (PreparedStatement del = conn.prepareStatement("DELETE FROM PUBLIC.HOLDFAST_PLANT_DAYS WHERE holdfast_id = ?")) {
                del.setInt(1, h.getId());
                del.executeUpdate();
            }
            insertBuildingsAndPlantDays(conn, h);
    ...
    public void deleteHoldfast(String groupName) {
        try (Connection conn = getConnection()) {
            // Get id first
            int id = -1;
            try (PreparedStatement ps = conn.prepareStatement("SELECT id FROM PUBLIC.HOLDFASTS WHERE group_name = ?")) {
                ps.setString(1, groupName);
                try (ResultSet rs = ps.executeQuery()) {
            ...
            if (id == -1) return;
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM PUBLIC.HOLDFAST_EVENTS WHERE holdfast_id = ?")) {
                ps.setInt(1, id); ps.executeUpdate();
            }
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM PUBLIC.HOLDFAST_PLANT_DAYS WHERE holdfast_id = ?")) {
                ps.setInt(1, id); ps.executeUpdate();
            }
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM PUBLIC.HOLDFAST_BUILDINGS WHERE holdfast_id = ?")) {
                ps.setInt(1, id); ps.executeUpdate();
            }
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM PUBLIC.HOLDFASTS WHERE id = ?")) {
                ps.setInt(1, id); ps.executeUpdate();
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    @Override
    public void addHoldfastEvent(int holdfastId, int dayNumber, String message) {
        String sql = "INSERT INTO PUBLIC.HOLDFAST_EVENTS (holdfast_id, day_number, message) VALUES (?, ?, ?)";
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, holdfastId);
            ps.setInt(2, dayNumber);
            ps.setString(3, message != null && message.length() > 2000 ? message.substring(0, 2000) : message);
            ps.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    @Override
    public List<Map<String, Object>> getHoldfastEvents(int holdfastId) {
        String sql = "SELECT id, day_number, message FROM PUBLIC.HOLDFAST_EVENTS WHERE holdfast_id = ? ORDER BY id ASC";
        ...
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    row.put("day", rs.getInt("day_number"));
    ...
    private List<Integer> splitInts(String s) {
