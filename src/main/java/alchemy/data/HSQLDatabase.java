...
public class HSQLDatabase implements IStubDatabase {
 ...
 // my public IP 1
    ...
    public void addPlayer(Player player) {
        String sql = "INSERT INTO PUBLIC.PLAYERS (id, username, password, level, email, display_name, registration_date, last_login_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = getConnection();
                PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            pstmt.setString(6, player.getDisplayName());
            pstmt.setTimestamp(7, player.getRegistrationDate() != null ? Timestamp.valueOf(player.getRegistrationDate()) : null);
            pstmt.setTimestamp(8, player.getLastLoginDate() != null ? Timestamp.valueOf(player.getLastLoginDate()) : null);
            pstmt.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void updatePlayerLastLogin(int playerId, LocalDateTime lastLoginDate) {
        String sql = "UPDATE PUBLIC.PLAYERS SET last_login_date = ? WHERE id = ?";
        try (Connection conn = getConnection();
                PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setTimestamp(1, lastLoginDate != null ? Timestamp.valueOf(lastLoginDate) : null);
            pstmt.setInt(2, playerId);
            pstmt.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Error updating last login date for player ID: " + playerId);
            e.printStackTrace();
        }
    }
